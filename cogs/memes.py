...
class Memes(commands.Cog):
    ...
    async def motd(self, ctx: KurisuContext):
        """got milk?"""
        motd_list = [f"{str(self.flushedball)}", f"{str(self.flushedsquish)}", f"{str(self.staticbean)}", f"{str(self.flushedeyes)}", "ur mom lol", "hot dogs are sandwiches dont @ me", "have you had a coffee today?", "bird app bad", "imagine having opinions in current year", "based", "pog", "ratio", "remember to moisturize today!", "drink some water u idiot", "take ur meds", "do you like neapolitan ice cream?", "cope", "nentondon swonch", "xnoe at 11pm moment", "nh was a mistsake", "you have pronouns dipshit, it came free with your fucking language", "my brother in christ you use linux", "btw i use arch", "has junko had a name change today?", "got milk?", "got pilk?", "it has been 0 days since eip broke me", "ETA WEN PLS"]
