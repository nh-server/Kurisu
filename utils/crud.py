...
# Operations for managing staff
async def add_staff(user_id: int, position: str) -> None:
    await add_dbmember_if_not_exist(user_id)
    staff = await get_staff_member(user_id) or await get_helper(user_id)
    if staff:
...
async def remove_helper(user_id: int) -> None:
...
# Operations for managing permanent roles
...
# Operations for managing timed restrictions
...
# Operations for managing timed roles
...
async def remove_timed_role(user_id: int, role_id: int) -> None:
...
# Operations for managing softbans
...
# Operations for managing the watchlist
async def add_watch(user_id: int) -> None:
...
async def get_watch_list() -> list[models.Member]:
    return await models.Member.query.where(models.Member.watched.isnot(False)).gino.all()
...
async def add_friendcode_3ds(user_id: int, fc: int) -> None:
    await add_dbmember_if_not_exist(user_id)
...
async def add_friendcode_switch(user_id: int, fc: int) -> None:
    await add_dbmember_if_not_exist(user_id)
    if fcs := await get_friendcode(user_id):
...
async def delete_friendcode_3ds(user_id: int) -> None:
...
async def delete_friendcode_switch(user_id: int) -> None:
    friendcodes = await get_friendcode(user_id)
    if friendcodes:
...
# Operations for managing the rules
async def add_rule(number: int, description: str) -> None:
...
async def delete_rule(number: int) -> None:
    rule = await get_rule(number)
    if rule:
        await rule.delete()


async def edit_rule(number: int, description: str) -> None:
    rule = await get_rule(number)
    if rule:
        await rule.update(description=description).apply()


# Operations for managing reminders
async def add_reminder(date: datetime.datetime, author: int, reminder: str) -> None:
...
async def add_citizen(citizen_id) -> models.Citizen:
...
async def add_social_credit(citizen_id: int, social_credit: int) -> None:
...
async def remove_vote_view(view_id: int) -> None:
    view = await models.VoteView.get(view_id)
    await view.delete()


async def add_voteview_vote(view_id: int, voter_id: int, option: str) -> None:
...
async def get_voteview_vote(view_id: int, voter_id: int) -> Optional[models.VoteViewVote]:
    return await models.VoteViewVote.get((view_id, voter_id))


async def get_voteview_votes(view_id: int) -> list[models.VoteViewVote]:
