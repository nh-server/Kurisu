...
def check_for_position(*, helper=False, staff=False, op=False, superop=False, owner=False):
    ...
    async def predicate(ctx: commands.Context):
        ...
        if helper:
            valid = valid or (await get_role('helpers-role')) in roles
        if staff:
            valid = valid or (await get_role('staff-role')) in roles
        if op:
            valid = valid or ((await get_role('op-role')) in roles
                               or (await get_role('superop-role')) in roles
                               or (await get_role('owner-role')) in roles)
        if superop:
            valid = valid or ((await get_role('superop-role')) in roles
                               or (await get_role('owner-role')) in roles)
        if owner:
            valid = valid or (await get_role('owner-role')) in roles
        return valid
