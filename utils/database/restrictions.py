...
tables = {'restrictions': OrderedDict((('id', 'int'), ('user_id', 'int'), ('type', 'text'), ('end_date', ''), ('alerted', 'bool'))),
          'softbans': OrderedDict((('id', 'int'), ('user', 'int'), ('issuer', 'int'), ('reason', 'str')))}
...
class RestrictionsDatabaseManager(BaseDatabaseManager, tables=tables):
    ...
    async def remove_restriction(self, user_id: int, restriction: str) -> int:
        ...
        assert isinstance(user_id, int)
        res = await self._delete('restrictions', user_id=user_id, restriction=restriction)
        if res:
    ...
    async def set_timed_restriction_alert(self, restriction_id):
        await self._update('restrictions', {'alerted': True}, id=restriction_id)

    async def add_softban(self, user_id, issuer_id, reason: str):
        await self.bot.configuration.add_member(user_id)
        now = time_snowflake(datetime.now())
        await self._insert('softbans', id=now, user=user_id, issuer=issuer_id, reason=reason)
    ...
    async def remove_softban(self, user_id: int):
        await self._delete('softbans', user=user_id)
