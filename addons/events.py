...
class Events:
    ...
    async def scan_message(self, message):
        ...
        if contains_fs_repo_url != None:
            ...
            await self.bot.send_message(self.bot.mods_channel, "**Bad URL**: {} posted a freeShop Repo URL in {} (message deleted)".format(message.author.mention, message.channel.mention), embed=embed)
        if contains_piracy_site_mention:
            await self.bot.delete_message(message)
            await self.bot.send_message(self.bot.mods_channel, "**Bad site**: {} mentioned a piracy site in {} (message deleted)".format(message.author.mention, message.channel.mention), embed=embed)
