...
and helpers can be found in #welcome-and-rules if you don't know who they are.
    ...
    async def stock(self, ctx, console=None):
        ...
        if self.check_console(console, channel_name, '3ds'):
            ...
            embed.add_field(name="Hardware Independent", value="The latest update to common guide methods mean that the best method for you now depends on your firmware version. Please read the [guide](https://3ds.hacks.guide/get-started).", inline=False)
            await ctx.send(embed=embed)
