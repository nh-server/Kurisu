    ...
    async def guide(self, ctx, *, consoles=""):
        ...
        for x in consoleslist:
            if self.check_console(x, channelName, '3ds'):
                ...
                await ctx.send(embed=embed)
                continue
            if self.check_console(x, channelName, ('wiiu',)):
                ...
                await ctx.send(embed=embed)
                continue
            if self.check_console(x, channelName, ('vwii',)):
                ...
                await ctx.send(embed=embed)
                continue
            if self.check_console(x, channelName, ('switch', 'nx', 'ns')):
                ...
                await ctx.send(embed=embed)
                continue
            if self.check_console(x, channelName, ('legacy', 'wii')):
                embed = discord.Embed(title="Guide", color=discord.Color(0x009AC7))
...
and helpers can be found in #welcome-and-rules if you don't know who they are.
    ...
    async def vguides(self, ctx):
        ...
        embed.description = cleandoc("""
                ...
                - Tough to update after the video is uploaded, due to how videos work, it is impossible to update a specific part of it without creating an entirely new video
    ...
    async def newver(self, ctx, console=None):
        ...
        if self.check_console(console, channelName, '3ds'):
            ...
            *Last edited: December 3, 2019*
            ...
            embed.description = cleandoc("""
            Currently, the latest Switch system firmware is `10.2.0`.

            If your Switch is **unpatched and can access RCM**:
            Atmosphere and Hekate currently support 10.2.0, and unpatched units will always be hackable.
...
are not on 11.3, use [this version of safehax.](https://github.com/TiniVi/safehax/releases/tag/r19)
    ...
    async def vc(self, ctx, *, consoles=""):
        ...
        if not isinstance(ctx.channel, discord.DMChannel):
            channelName = ctx.channel.name
        if not consoleslist:
            if channelName.startswith(injects):
                consoleslist = ['auto']
            else:
...
One way to fix this is by using an y-cable to connect the HDD to two USB ports.
    ...
    async def nsupdate(self, ctx):
        ...
        await self.simple_embed(ctx, cleandoc("""
                                     ...
                                     **Atmosphere 0.14.4 (latest release)**
                                     Supports up to firmware 10.2.0.
